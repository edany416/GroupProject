...
class FirebaseManager {
   ...
   private func removeUserFromHouse(completion: @escaping () -> ()) {
        ...
        populateBasicInfo(completion: { (isInHouse) in
            if isInHouse {
                self.populateHouseBasedInfo(completion: {() in
                    print("finished populating all info/User is in house")
                    completion()
                })
            } else {
                print("finished populating all info/User not in house")
                completion()
            }
        })
    }
